# -*- coding: utf-8 -*-
"""
aerosol_ext_coeff_visibility_fernald_inversion_hourly_xlsx_batch.py

Batch Fernald inversion driven by hourly visibility from an Excel table using nearest-hour block matching.

Main features
-------------
1. Read all *_preprocessed.npz files under date subfolders produced by pre_process.py.
2. Use raw timestamp saved in each npz as the only time basis.
   The timestamp is UTC in seconds and will be converted to UTC+8.
3. Read hourly visibility from an xlsx table:
   - row 1: categories
   - row 2: units
   - row 3 onward: data
   - column A: time string in local time, format MM-DD HH:00
   - column J: visibility, unit km
   - column N/O/P: AQI / PM2.5 / PM10 index (saved as metadata if available)
4. Missing hourly visibility values are linearly interpolated only within the hourly table.
5. Each radar profile is assigned the visibility of the nearest whole hour
   (block matching, e.g. 07:30–08:29:59 -> 08:00 where applicable),
   instead of second-level linear interpolation.
6. Output:
   - single-radial time-height extinction curtain with blank gaps for non-acquisition periods
   - file-internal VAD-16 time-height extinction curtain with blank gaps
   - target 16-radial group figure (16 light profiles + 1 dark mean profile), if target time is configured
   - one result npz for each date folder

Notes
-----
- Non-acquisition periods of lidar data are rendered as blank segments in curtains.
  No interpolation is performed across lidar acquisition gaps.
- Visibility-table missing hours are interpolated because they are reference-data gaps,
  not lidar-observation gaps.
- After the hourly table is completed, radar profiles use nearest-hour block matching
  rather than second-level linear interpolation.
"""
...
# User settings
# =========================================================
# Root directory that contains multiple date subfolders of preprocessed npz files.
PREPROCESSED_ROOT = Path(r"E:\测风组实验数据\气溶胶反演\预处理")

# Hourly weather table (xlsx)
VISIBILITY_XLSX_PATH = Path(r"F:\3220240787\DataShareClub\weather_data\beijing_weather.xlsx")

# Output root
OUTPUT_ROOT = Path(r"E:\测风组实验数据\气溶胶反演\fernald_output_hourly_blockmatch")

# Optional: process only selected date folders.
# Set to None to auto-scan all subfolders under PREPROCESSED_ROOT.
DATE_FOLDER_WHITELIST: Optional[list[str]] = None

# Optional target local time (UTC+8) for selected dates.
# If a date is absent from this dict, the script will skip the target-group figure for that date.
TARGET_LOCAL_TIME_MAP: dict[str, str] = {
    "2024-10-26": "2024-10-26 20:00:00",
    # "2026-04-19": "2026-04-19 20:00:00",
    # "2026-04-19": "2026-04-19 23:00:00",
    # "2026-04-20": "2026-04-20 00:00:00",
    # "2026-04-20": "2026-04-20 03:00:00"
    # "2026-04-20": "2026-04-20 20:00:00",
}
...
def parse_target_local_time(target_local_time_str: str) -> pd.Timestamp:
    ts = pd.Timestamp(target_local_time_str)
...
def load_hourly_weather_table_for_year(
    ...
    Returns a dataframe indexed by local datetime (Asia/Shanghai).
    """
    ...
    data = raw.iloc[2:].copy()  # row 3 onward are data rows
...
def build_hourly_weather_for_date(
    ...
    Returns dataframe indexed by hourly local datetime with columns:
    - visibility_km
    - AQI
    - PM25_index
    - PM10_index
    ...
    # Visibility: interpolate missing internal gaps in time; fill edges if needed.
    vis = df_day["visibility_km"].astype(float)
    vis = vis.interpolate(method="time", limit_direction="both")
    df_day["visibility_km"] = vis

    # AQI / PM indices are optional metadata; keep raw and also time-interpolate if possible.
...
def assign_nearest_hour_visibility_to_times(
    hourly_visibility_km: pd.Series,
    target_times_local: pd.DatetimeIndex,
) -> np.ndarray:
    """
    Assign the visibility of the nearest whole hour to each radar profile time.

    The hourly table is assumed to have already been gap-filled at the hourly level.
    This avoids introducing artificial second-level variability in the Fernald
    reference condition.

    Example:
    - 07:30:00 to 08:29:59 -> 08:00 visibility
    - 08:30:00 to 09:29:59 -> 09:00 visibility

    For edge hours at the beginning/end of the day, the nearest available hour
    ...
    if hourly_visibility_km.empty:
        raise ValueError("hourly_visibility_km is empty")

    hour_index = pd.DatetimeIndex(hourly_visibility_km.index)
    hour_ns = hour_index.asi8
    out = np.empty(len(target_times_local), dtype=np.float64)

    for i, t in enumerate(target_times_local):
        t_ns = t.value
        pos = np.searchsorted(hour_ns, t_ns)

        if pos == 0:
            idx = 0
        elif pos >= len(hour_ns):
            idx = len(hour_ns) - 1
        else:
            left_idx = pos - 1
            right_idx = pos
            left_diff = abs(t_ns - hour_ns[left_idx])
            right_diff = abs(hour_ns[right_idx] - t_ns)
            idx = left_idx if left_diff <= right_diff else right_idx

        out[i] = float(hourly_visibility_km.iloc[idx])
...
def average_one_file_by_vad_cycle(
    ...
    """
    VAD-16 averaging within one file only.
...
def plot_target_vad16_profiles(
    ...
    - mean profile in a dark thick line
    """
...
def process_one_date_folder(preprocessed_date_dir: str | Path, output_root: str | Path):
    ...
    # Nearest-hour visibility and reference values for single-radial profiles
    vis_single_km = assign_nearest_hour_visibility_to_times(hourly_visibility_km, time_local_single)
    ...
    vis_vad_km = assign_nearest_hour_visibility_to_times(hourly_visibility_km, time_local_vad)
    ...
    # 7. Output curtains
    # -----------------------------------------------------
    curtain_single_png = out_dir / f"{date_key}_time_height_single_hourly_blockmatch.png"
    curtain_vad_png = out_dir / f"{date_key}_time_height_vad16_file_internal_hourly_blockmatch.png"
    ...
    # -----------------------------------------------------
    # 8. Optional target 16-radial group figure
    # -----------------------------------------------------
    ...
    beta_group_16 = None
    alpha_group_16 = None
    beta_group_mean = None
    alpha_group_mean = None
    vis_group_16_km = None
    vis_group_mean_km = None
    ...
    if target_local_time_str is not None:
        ...
        # Nearest-hour visibility for the 16 individual profiles and the mean profile
        vis_group_16_km = assign_nearest_hour_visibility_to_times(
            hourly_visibility_km,
            target_group["time_local_group_16"],
        )
        _, beta_a_ref_group_16, _, _ = visibility_to_reference_beta_alpha_from_km(vis_group_16_km)

        vis_group_mean_km = assign_nearest_hour_visibility_to_times(
            hourly_visibility_km,
            pd.DatetimeIndex([target_group["time_local_group_mean"]]),
        )[0]
        _, beta_a_ref_group_mean, _, _ = visibility_to_reference_beta_alpha_from_km(np.array([vis_group_mean_km]))
        ...
        target_group_png = out_dir / f"{date_key}_target_vad16_profiles_hourly_blockmatch.png"
    ...
    # 9. Save results
    # -----------------------------------------------------
    out_npz = out_dir / f"{date_key}_fernald_inversion_hourly_blockmatch.npz"
    ...
    if target_local_time_str is not None and target_group is not None:
        save_dict.update({
            ...
            "visibility_group_16_km": vis_group_16_km,
            "visibility_group_mean_km": np.array(vis_group_mean_km, dtype=np.float64),
            "x_group_16": target_group["x_group_16"],
            "x_group_mean": target_group["x_group_mean"],
    ...
    # -----------------------------------------------------
    print("=" * 78)
    print("Nearest-hour-block-matched Fernald inversion finished")
    ...
    if target_local_time_str is not None and target_group is not None:
        ...
        print("Target-group figure: skipped (no target time configured for this date)")
