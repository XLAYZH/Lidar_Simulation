...
4. Missing hourly visibility values are linearly interpolated in time.
5. Visibility is then linearly interpolated from the hourly table to each radar profile time,
   so the Fernald reference value varies with time during the day.
6. Output:
...
# Output root
OUTPUT_ROOT = Path(r"E:\测风组实验数据\气溶胶反演\fernald_output_hourly")
...
def interpolate_hourly_visibility_to_times(
    ...
    """
    Interpolate hourly visibility to arbitrary profile times (local time, UTC+8).
    This improves temporal precision relative to daily-mean inversion.
    """
...
def process_one_date_folder(preprocessed_date_dir: str | Path, output_root: str | Path):
    ...
    # Time-varying visibility and reference values for single-radial profiles
    ...
    if target_local_time_str is not None:
        ...
        # Time-varying visibility for the 16 individual profiles and the mean profile
        ...
        target_group_png = out_dir / f"{date_key}_target_vad16_profiles_hourly_vis.png"
    ...
    # 9. Save results
    # -----------------------------------------------------
    out_npz = out_dir / f"{date_key}_fernald_inversion_hourly_vis.npz"
